...
class BaseMediaExtractor(PipelineStep):
    ...
    def run(self, data: DocumentsPipeline, rank: int = 0, world_size: int = 1) -> DocumentsPipeline:
        ...
        """
        with (
            ExtractorSandbox(timeout=self.timeout, warmup_text=(None, None)) as extractor,
            self.exclusion_writer
            if self.exclusion_writer
            else contextlib.nullcontext() as exclusion_writer,
        ):
            for doc in data:
                    ...
                    for media in doc.media:
                        try:
                            ...
                            if metadata.get("extraction_error"):
                                logger.warning(
                                    f"❌ Error {metadata['extraction_error']} while cleaning record text. Skipping record."
                                )
                ...
                if all(media.metadata.get("extraction_failed", False) for media in doc.media):
                    for media in doc.media:
                        logger.warning(
                            f"❌ Media {media.id} extraction failed with error "
                            f"{media.metadata['extraction_failed']}"
                        )
